...
class Pirate < ActiveRecord::Base
  ...
  def self.report_builder(pirate1, pirate2,shield_or_parrot1,shield_or_parrot2)
    ...
    while hp1.positive? && hp2.positive?
      report.push "Round n°"+ i.to_s
      report.push 'Il reste ' + hp1.to_s + "hp \xC3\xA0 " + name1 + ' et ' +
                  hp2.to_s + "hp \xC3\xA0 " + name2



      if shield_or_parrot2 == 1 and pirate2.strengh_challenge?
      ...
      if shield_or_parrot1 == 1 and pirate1.strengh_challenge?
      ...
      report.push 'chaque assaillant recule et se fronce les sourcils pour lancer un sort de soin...'
      report.push "Ils se souviennent subitement qu'ils sont incapable
                      de lancer des sorts puisque ce sont des pirates"
      report.push 'Confus, ils partatagent une bouteille de rhum du Bord-des-Sables '
      report.push name1+' récupère ' + int1.to_s + "hp et " + name2 + ' récupère ' +
                      int2.to_s + "hp"

      if shield_or_parrot1 == 2 and pirate1.int_challenge?
        report.push "le perroquet de "+name1+" se change en party parrot, cela blesse "+ name2
        report.push "il perd "+int1+" hp"
        hp2 -= int1
      end

      if shield_or_parrot2 == 2 and pirate2.int_challenge?
        report.push "le perroquet de "+name2+" se change en party parrot, cela blesse "+ name1
